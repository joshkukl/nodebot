...
import { CoinbasePro, OrderBookLevel, OrderBookLevel3 } from 'coinbase-pro-node';
import { FullOrderBook } from './FullOrderBook';
export class BookKeeper {
    ...
    getSnapshot(client: CoinbasePro, firstsequence: number) {
    ...
    // A level 2 orderbook could be kept much easily by subscribing to the "level 2" channel.
        client.rest.product.getProductOrderBook(this.product, { level: OrderBookLevel.FULL_ORDER_BOOK } )
            .then((result: OrderBookLevel3) => {
    ...
    // of the first message received by the Websocket feed. This ensure no messages are missed.
                if (result.sequence >= firstsequence - 1 ) {
                    ...
                    setTimeout(() =>  {
                        this.getSnapshot(client, firstsequence);
                    }, 1000);
                }
            });
    };
    buildOrderBook(snapshot: OrderBookLevel3): boolean {
        this.book.sequence = snapshot.sequence;
        console.log("GETTING ORDERBOOK: " + this.book.sequence);
        this.book.bids = new Map(snapshot.bids.map((i: any[]) => [i[2], { price: i[0], size: i[1] }]));
        this.book.asks = new Map(snapshot.asks.map((i: any[]) => [i[2], { price: i[0], size: i[1] }]));
        this.isready = true;
    ...
    addOrder(message: any): void {
        let order = { price: message.price, size: message.remaining_size };
    ...
    changeOrder(message: any): void {
        let order = { price: message.price, size: message.new_size };
        let order_id = message.order_id; 
        this.book.sequence = message.sequence;
        if(message.side == 'buy') {
            this.book.bids.set(order_id, order);
